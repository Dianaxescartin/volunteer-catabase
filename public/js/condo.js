...
async function getCatInfoByCondo() {
    try {
        const response = await fetch(`http://localhost:8080/api/cats/condo/${condoNumber}`);
...
function createCatCard(cat) {
    ...
    condoNumber.innerHTML = `<strong>Condo: </strong><span>${cat.condoNumber}</span>`;
...
async function renderCatsCards() {
    try {
        const catInfo = await getCatInfoByCondo();
        catInfo.forEach((cat) =>
            createCatCard(cat)
        );
    }
